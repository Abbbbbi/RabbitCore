...
const char * const BITCOIN_CONF_FILENAME = "rabbit.conf";
const char * const BITCOIN_PID_FILENAME = "rabbitd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "rabbit" is a composite category enabling all Rabbit-related debug output
            if(ptrCategory->count(string("rabbit"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RabbitCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RabbitCore
    // Mac: ~/Library/Application Support/RabbitCore
    // Unix: ~/.rabbitcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RabbitCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RabbitCore";
#else
    // Unix
    return pathRet / ".RabbitCore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty rabbit.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("rabbit.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override rabbit.conf
